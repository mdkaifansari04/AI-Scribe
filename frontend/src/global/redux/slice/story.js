...
const option = {
  style: {
    background:
      localStorage.getItem("aiScribeTheme") === "dark" ? "#333" : "#fff",
...
const story = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getDashboardData.fulfilled, (state, action) => {
      ...
      if (action.payload.topVotedStories)
        state.topVotedStories = action.payload.topVotedStories;
      if (action.payload.userGeneratedStories)
        state.userGeneratedStories = action.payload.userGeneratedStories;
    }),
      ...
      builder.addCase(deleteStory.fulfilled, (state, action) => {
        ...
        state.topVotedStories = state.topVotedStories.filter(
          (story) => story._id !== action.payload.story._id
        );
        state.userGeneratedStories = state.userGeneratedStories.filter(
          (story) => story._id !== action.payload.story._id
        );
      }),
      ...
      builder.addCase(saveStory.fulfilled, (state, action) => {
        ...
        state.userGeneratedStories = [
          ...state.userGeneratedStories,
          action.payload.story,
        ];
        if (action.payload.story.rating > 2)
          state.topVotedStories = [
            ...state.topVotedStories,
            action.payload.story,
          ];
      }),
