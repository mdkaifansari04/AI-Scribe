...
function Form() {
  ...
  const handleChange = (name, newValue) => {
    ...
    if (
      promptData.story
        ? promptData.story.length > 10
          ? true
          : false
        : false && promptData.storyType && promptData.wordLimit
    ) {
      setDisableBtn(false);
    } else {
      setDisableBtn(true);
      ...
      <form className="mx-auto my-6 w-[90%] max-w-screen-lg dark:text-white">
        <div className="flex flex-col gap-10 w-full">
          <Input
            ...
            value={promptData.story}
            variant="static"
            label="Prompt"
            placeholder="A city where everyone can fly..."
            required
          />
          <Input
            color={themeColor}
          ...
          <Select
            ...
            value={promptData.suspense} // Bind the value to your state
            className="custom-select-menu"
            variant="static"
            label="Suspense"
          >
        ...
        <Button
          ...
          className="bg-[#0ABF8C] my-10"
          fullWidth
        >
          <span className="flex justify-center align-middle">
            {loading ? <Spinner className="h-3 w-3 mx-3 p-0" /> : "Generate"}
          </span>
        </Button>
      </form>
    </>
