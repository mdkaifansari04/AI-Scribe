...
export default function MainNavbar(props) {
  ...
  const choice = localStorage.getItem("aiScribeTheme")
    ? localStorage.getItem("aiScribeTheme")
    : "light";
  ...
  function ProfileMenu() {
    ...
    return (
      <Menu
        ...
        className="dark:bg-[#3A444E]"
      >
        <Tooltip
          className="bg-[#3d3d3ddd] text-white  dark:bg-[#dfdada] dark:text-black"
          content="Theme Mode"
          placement="right"
          animate={{ mount: { scale: 1, x: 0 }, unmount: { scale: 0, x: 25 } }}
        >
          <MenuHandler>
            <Button
              variant="text"
            ...
            >
              ...
              <Typography variant="h6" className="font-normal  dark:text-white">
                {localStorage.getItem("aiScribeUserName")}
                <br />
              </Typography>
              <MenuList className="dark:bg-[#3a444ec1] border-none">
                <MenuItem
                  onClick={setThemeToLight}
                  className="flex items-center gap-2  dark:text-white  dark:hover:text-black"
                >
                ...
                <MenuItem
                  onClick={setThemeToSystemPreference}
                  className="flex items-center gap-2 dark:text-white  dark:hover:text-black"
                >
            ...
            <Typography className="mr-4 flex align-middle justify-center cursor-pointer py-1.5 font-medium">
              <div className="logo-icon-container flex mr-5">
                <div className="logo w-10 mr-2">
                  <img
                    src="https://res.cloudinary.com/dngfmzv2g/image/upload/v1695238520/ai-scribe-website-favicon-color_rqk9oe.png"
                    alt=""
                  />
                </div>
                <Bars3BottomLeftIcon
                  onClick={openDrawer}
                  strokeWidth={2}
                  className="h-7 w-7 m-1 dark:text-white"
                />
              </div>
            </Typography>
          )}
